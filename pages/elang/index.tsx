...
export default function ElangPage(props: { result: string }) {
  ...
  const submitHandler = () => {
    const sentence = replaceSentence({
      sentence: rawText.toLocaleLowerCase(),
      isTranslating,
    });
  ...
  return (
    <>
      <Head>
        <title>E-Lang</title>
        <meta name="description" content={`${props.result}`} />
      </Head>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const { query } = context;
  const { translating, sentence } = query;
  if (!translating || !sentence) {
    return {
      props: {
        result: '',
      },
  ...
  const isTranslating = translating === 'true';
  const result = replaceSentence({
